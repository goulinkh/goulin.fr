...
function create-dev-container() {
	...
	lxc storage volume create docker $container_name
	lxc config device add $container_name docker disk source=docker path=/var/lib/docker
	...
	# optional: additional tools setup in new container
	# lxc exec $container_name -- curl -fsSL $setup_script | bash
}
```
## Conclusion
